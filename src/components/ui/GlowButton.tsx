...
const GlowButton = forwardRef<HTMLButtonElement, GlowButtonProps>(
  (
    ...
    if (variant === 'ghost') {
      return (
        <button
          ...
          {...props}
        >
          {loading && <span_ style={{ width:13, height:13, border:'2px solid currentColor', borderTopColor:'transparent', borderRadius:'50%', display:'inline-block', animation:'spin .6s linear infinite', marginRight:8 }}/>}
          {children}
        </button>
      )
    }
    ...
    return (
      ...
      >
        {loading && <span_ style={{ width:13, height:13, border:'2px solid currentColor', borderTopColor:'transparent', borderRadius:'50%', display:'inline-block', animation:'spin .6s linear infinite', marginRight:8 }}/>}
        {icon && !loading && <span_ style={{ marginRight:8, display:'inline-flex', alignItems:'center' }}>{icon}</span_>}
        {children}
      </button>
    )
  }
